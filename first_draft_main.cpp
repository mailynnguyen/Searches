...
void uniform_cost_search(Board currNode, vector<vector<int>> goal, vector<Board>& openList, vector<Board>& closedList) {
    ...
    if (openList.size() == 0) {
        ...
        return;
    } else if (currNode.getBoard() == goal) {
        ...
        for (int i = 0; i < 3 && !foundZero; i++) {
            for (int j = 0; j < 3 && !foundZero; j++) {
                if (currNode.getBoard()[i][j] == 0) {
                    ...
                    for (int i = 0; i < 3; i++) {
                        for (int j = 0; j < 3; j++) {
                            cout << currNode.getBoard()[i][j] << " ";
                        }
                    }
                    cout << endl;
                    cout << "cost: " << currNode.getCost() << endl;
        ...
        cout << "first in open-list: ";
        for (int i = 0; i < 3; i++) {
            for (int j = 0; j < 3; j++) {
                cout << openList[0].getBoard()[i][j] << " ";
            }
        }
        cout << endl;
        cout << "open-list node cost: " << openList[0].getCost() << endl;
